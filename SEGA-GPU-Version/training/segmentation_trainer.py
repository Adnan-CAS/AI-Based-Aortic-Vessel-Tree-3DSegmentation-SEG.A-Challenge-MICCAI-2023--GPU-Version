...
from evaluation import evaluation_sega as evf
...
class SegmentationTrainer():
    def __init__(self, **training_params : dict):
        ...
        # self.to_load_checkpoint_path : Union[str, pathlib.Path] = training_params['to_load_checkpoint_path']
    ...
    def log_losses(self, mode: str) -> None:
        dataset_size = len(self.training_dataloader.dataset) if mode == "training" else len(self.validation_dataloader.dataset)
        loss_value = self.acc_loss / dataset_size
        dice_value = self.acc_dice / dataset_size
        hausdorff_value = self.acc_hausdorff / dataset_size
        print(f"Iteration {self.current_iter}, Mode: {mode.capitalize()}, Loss: {loss_value}, Dice: {dice_value}, Hausdorff: {hausdorff_value}")
        self.logger.add_scalar(f"Loss/{mode.capitalize()}/loss", loss_value, self.current_iter)
        self.logger.add_scalar(f"Loss/{mode.capitalize()}/dice", dice_value, self.current_iter)
        self.logger.add_scalar(f"Loss/{mode.capitalize()}/hausdorff", hausdorff_value, self.current_iter)
        if self.log_time:
            self.logger.add_scalar(f"Time/{mode.capitalize()}/time_loading", self.acc_loading_time / dataset_size, self.current_iter)
            self.logger.add_scalar(f"Time/{mode.capitalize()}/time_augmentation", self.acc_augmentation_time / dataset_size, self.current_iter)
            self.logger.add_scalar(f"Time/{mode.capitalize()}/time_network", self.acc_network_time / dataset_size, self.current_iter)
    ...
    def log_images(self, mode : str) -> None:
        ...
        for (i, batch) in enumerate(dataloader):
            with tc.set_grad_enabled(False):
                ...
                image_path = os.path.join(p.logs_path, f"image_{i}.png")
                save_image(image, image_path)
                plt.close('all')
            if i >= self.number_of_images_to_log:
                break
        # for i in range(self.number_of_images_to_log):
        #     title = f"Mode: {mode.capitalize()} Case: {i}, Iter: {str(self.current_iter)} Dice: {dice}"
        #     # Assuming the logger object has a method named 'get_image' to retrieve the logged images
        #     image = self.logger.get_image(title)
    
        #     # Display the image using your preferred method (e.g., matplotlib or PIL)
        #     plt.figure()
        #     plt.imshow(image)
        #     plt.title(title)
        #     plt.show()
    ...
    #     dataloader = self.training_dataloader if mode == "training" else self.validation_dataloader
    #     for (i, batch) in enumerate(dataloader):
    #         # Extract input data, ground truth, and model output from the batch
    #         input_data, ground_truth, _ = batch
    #         input_data, ground_truth = input_data.type(self.dtype).to(self.device), ground_truth.type(self.dtype).to(self.device)
    #         output = self.model(input_data)

    #         # Visualize and log the images
    #         # Here you can use matplotlib to create visualizations and then log them using SummaryWriter
    #         # Use add_image() to log the visualizations with relevant information

    #         # Example pseudocode for visualizing and logging images
    #         fig, axs = plt.subplots(1, 3)
    #         axs[0].imshow(input_data[0].cpu().numpy())  # Input data
    #         axs[1].imshow(ground_truth[0].cpu().numpy())  # Ground truth
    #         axs[2].imshow(output[0].cpu().detach().numpy())  # Model output
    #         title = f"Mode: {mode.capitalize()}, Iter: {self.current_iter}, Sample: {i}"
    #         self.logger.add_image(title, fig, 0)  # Log the visualization using SummaryWriter
